#!/usr/bin/env groovy
/***
	Parse PDF with employees of Hacienda and generate JSON.
...
def lines = linesFromPdf(new File("../data/hacienda.pdf"))
...
static def filterLines(lines) {
	lines = lines.findAll { it.split(";").size() > 5 }
	lines
}
...
static def line2Json(line) {
	...
	def map = [:]
	map["nombre"] = fields[0]
	map["categoria"] = fields[1]
	map["cargo"] = fields[2]
	map["tipo"] = fields[3]
	map["sueldo"] = fields[4].replaceAll("\\.", "").toInteger()
	map["extra"] = fields[5].replaceAll("\\.", "").toInteger() + fields[6].replaceAll("\\.", "").toInteger()
	map["total"] = fields[4].replaceAll("\\.", "").toInteger() + fields[5].replaceAll("\\.", "").toInteger() + fields[6].replaceAll("\\.", "").toInteger()
	map["institucion"] = "Hacienda"
	builder(map)
	builder.toString()	
}
